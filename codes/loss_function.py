...
162) to ~656, i.e. roughly **19:1** max-to-min. Three options:
  ...
  1. ``LabelSmoothingCrossEntropy`` — plain CE with label smoothing. With 251
     fine-grained classes, smoothing curbs over-confidence and improves
     generalisation; pair it with sqrt-tempered class weights (see
     data_handler.compute_class_weights, scheme='sqrt_inv').

  2. ``WeightedCrossEntropyLoss`` — per-class weights (use the sqrt-tempered or
     effective-number weights, NOT raw inverse frequency, so the 34-image class
     does not get a ~20x gradient multiplier).
...
⚠️  Choose ONE place to correct class frequency: if a ``WeightedRandomSampler``
    is active in the DataLoader, pass ``weight=None`` / ``alpha=None`` here.
"""
...
class LabelSmoothingCrossEntropy(nn.Module):
    """
    Cross-entropy with label smoothing (optionally class-weighted).

    Label smoothing ``eps`` redistributes a little probability mass off the true
    class, which is helpful for 251 visually-similar food categories.
    """

    def __init__(self, eps: float = 0.1, weight: torch.Tensor | None = None, reduction: str = "mean") -> None:
        super().__init__()
        self.eps = eps
...
class WeightedCrossEntropyLoss(nn.Module):
    """Standard CE with per-class inverse-frequency weights (+ optional smoothing)."""

    def __init__(self, weight: torch.Tensor | None = None, label_smoothing: float = 0.0, reduction: str = "mean") -> None:
        super().__init__()
        self.register_buffer("weight", weight)
...
def build_criterion(loss_type: str, class_weights: torch.Tensor | None = None, gamma: float = 2.0, label_smoothing: float = 0.1) -> nn.Module:
    ...
    Args:
        ...
        class_weights   : compute_class_weights(train_df).to(device), or None
                          when a WeightedRandomSampler is active.
        gamma           : focal focusing parameter (ignored otherwise).
        label_smoothing : smoothing eps for the CE-based losses.
    """
    if loss_type == "ce":
        return LabelSmoothingCrossEntropy(eps=label_smoothing, weight=class_weights)
